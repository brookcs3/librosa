...
# transform (STFT), which can be useful for visualizing how the frequency content of a
# signal changes over time.
# This kind of visualization is known as a **spectrogram**, and you've probably encountered
# these before.
...
# (which defaults to 80, but let's set it to 120).
# Finally, we can choose a different colormap by setting `cmap=`.
...
# the last example.
#
# Explicitly setting `cmap=None` will fall back to the default colormap in your matplotlib
# configuration.
...
#     frequency-by-frequency plots, etc.
